...
export class SecretObfuscator {
	...
	#collectRegexMatches(
		...
		for (;;) {
			...
			// "I" — generated by a PRIOR obfuscate() call) must not re-redact around
			// that placeholder, or a prior one-way replacement marker grows on
			// re-entry (`#…#REDACTED` -> `#…#REDACTEDDACTED`); obfuscate() runs on its
			// own output every turn (convertToLlm + transformProviderContext) and must
			// be a fixed point. But dropping the whole match leaks genuinely-new
			// surrounding bytes — e.g. a regex added after a placeholder was persisted,
			// so `api_key=#…#XYZ` re-enters with `XYZ` still raw. So keep the prior
			// placeholder atomic, yet still redact the surrounding bytes when they
			// independently satisfy the regex. That never holds for obfuscate()'s own
			// output (its surrounding bytes are already redacted, so re-matching needs
			// the placeholder to bridge them — and bridged-only matches are skipped),
			// preserving the fixed point, while new raw input is redacted.
			...
			if (overlapsInputPlaceholder) {
				...
				regex.lastIndex = 0;
				const outsideIndependentlyMatches = outside.length > 0 && regex.test(outside);
